...
function ScoreAnimeMovies() {
    const [jugar, setJugar] = useState(false);
    const [puntaje, setPuntaje] = useState(0);
    const [perdio, setPerdio] = useState(false);
    const { movie1, movie2, getRandomAnimeMovie1, getRandomAnimeMovie2 } = estadoMovies();
    ...
    const comenzarJuego = async () => {
        setPuntaje(0);
        await getRandomAnimeMovie1();
        await getRandomAnimeMovie2();
        setJugar(true);
    };

    useEffect(() => {
        const puntajeMasAlto = localStorage.getItem("HighScoreAnimeMovie") || 0;
        if (puntaje > puntajeMasAlto) {
            localStorage.setItem("HighScoreAnimeMovie", puntaje);
        }
    }, [puntaje]);
    ...
    return (
        <div className="h-screen w-screen flex flex-col items-center justify-center" style={{ backgroundColor: '#2e51a2' }}>
            {!jugar ? (
                ...
                <>
                    <div className="flex flex-col items-center w-[600px] h-[375px]">
                        ...
                        <h1 className="text-3xl font-bold mt-3">Adivina qué película Anime tiene una calificación más alta en IMDb</h1>
                        <h2 className="mt-3">Un juego de mayor o menor usando estadísticas de IMDb. Datos actualizados en 2025.</h2>
                    </div>
                    ...
                    < div className="w-full h-screen flex items-center justify-around">
                        ...
                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center space-y-4 z-20">
                            ...
                            <div className="bg-black text-white text-xl p-1 rounded">
                                ...
                                <h2>Puntaje Mas Alto: {localStorage.getItem("HighScoreAnimeMovie") || 0}</h2>
                            </div>
                        </div>
                        {/* Película 1 */}
                        <button
...
export default ScoreAnimeMovies;
